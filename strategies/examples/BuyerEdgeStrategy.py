...
# ── Scan universe ────────────────────────────────────────────────────────────
# SCANNED_UNDERLYINGS_CSV  : comma-separated list of symbols the bot actively
#   scans for signals and places orders on.  Add or remove symbols here.
#   Override at runtime: export UNDERLYINGS="NIFTY,BANKNIFTY"
SCANNED_UNDERLYINGS_CSV = os.getenv(
    "UNDERLYINGS",
    "NIFTY,BANKNIFTY,FINNIFTY,RELIANCE,HDFCBANK,ICICIBANK,SBIN,INFY,TCS",
)
# Deduped, whitespace-cleaned final scan list (order not guaranteed — set-derived).
UNDERLYINGS: list[str] = sorted(set(
    u.strip() for u in SCANNED_UNDERLYINGS_CSV.split(",") if u.strip()
))
...
# ── Index classifier ──────────────────────────────────────────────────────────
# INDEX_UNDERLYINGS_CSV  : master list of ALL known index underlyings.
#   Symbols here are routed via INDEX_EXCHANGE (NSE_INDEX/BSE_INDEX) instead of
#   SPOT_EXCHANGE for quotes, option chain, synthetic futures, and Greeks calls.
#   This list is intentionally broader than UNDERLYINGS so new index symbols can
#   be added to the scan list without also updating the classifier.
#   Override at runtime: export INDEX_UNDERLYINGS="NIFTY,BANKNIFTY,SENSEX"
INDEX_UNDERLYINGS_CSV = os.getenv(
    "INDEX_UNDERLYINGS",
    "NIFTY,BANKNIFTY,FINNIFTY,MIDCPNIFTY,SENSEX,BANKEX,NIFTYNXT50",
)
# frozenset for O(1) membership checks in _underlying_exchange() hot path.
INDEX_UNDERLYINGS: frozenset[str] = frozenset(
    u.strip() for u in INDEX_UNDERLYINGS_CSV.split(",") if u.strip()
)
INDEX_EXCHANGE = os.getenv("INDEX_EXCHANGE", "NSE_INDEX")   # NSE_INDEX or BSE_INDEX

# Relationship invariant (logged at startup, not a hard stop):
#   Every index symbol in UNDERLYINGS should appear in INDEX_UNDERLYINGS.
#   Stocks in UNDERLYINGS that are absent from INDEX_UNDERLYINGS are fine —
#   they are routed via SPOT_EXCHANGE automatically.
_unclassified = [s for s in UNDERLYINGS if s not in INDEX_UNDERLYINGS and s not in (
    "RELIANCE", "HDFCBANK", "ICICIBANK", "SBIN", "INFY", "TCS",
    # add any other known equity underlyings here so the check stays meaningful
)]
if _unclassified:
    print(
        ...
        f"SPOT_EXCHANGE ({SPOT_EXCHANGE}) which may cause bad data. "
...
class OptionsBuyerEdgeBot:
    """Multi-layer options buyer-edge bot. Each underlying scanned independently."""

    def __init__(self):
        self.client = api(api_key=API_KEY, host=API_HOST, ws_url=WS_URL)

        self.strategy_name = os.getenv("STRATEGY_NAME", "OptionsBuyerEdgeBot")
        ...
        self.last_entry_times: dict[str, datetime] = {}  # per-underlying cooldown clock
    ...
    def _check_risk_gates(self, symbol: str = "") -> tuple[bool, str]:
        """
        ...
        # 3. Entry cooldown — per-underlying independent clock.
        #    NIFTY entering does NOT block BANKNIFTY; each symbol tracks its own last entry time.
        if ENTRY_COOLDOWN_SECS > 0 and symbol:
            last_t = self.last_entry_times.get(symbol)
    ...
    def _register_filled_entry(
        ...
        with self.state_lock:
            ...
            self.last_entry_times[underlying] = datetime.now()
        ...
        if TRAIL_SL_MODE in ("spot", "both"):
            spot_activate_addon = round(reward_dist * TRAIL_ACTIVATE_AT_PCT / 100, 1)
            spot_activate_at    = round(spot + spot_activate_addon, 1)
            print(f"        Spot entry {spot:.1f} | reward dist {reward_dist:.1f} pts | "
                  f"spot trail activates at {spot_activate_at:.1f} (+{spot_activate_addon:.1f} pts)")
        if TRAIL_SL_MODE in ("premium", "both"):
            prem_reward   = PREMIUM_TARGET_PTS
            activate_addon = round(prem_reward * TRAIL_ACTIVATE_AT_PCT / 100, 2)
            activate_at    = round(executed + activate_addon, 2)
            print(f"        Premium entry ₹{executed:.2f} | prem reward ₹{prem_reward:.2f} pts (fixed) | "
                  f"trail activates at ₹{activate_at:.2f} (+₹{activate_addon:.2f})")
    ...
    def _scan_underlying(self, symbol: str):
        ...
        # Pass symbol so the per-underlying cooldown clock is checked correctly.
        allowed, gate_reason = self._check_risk_gates(symbol)
        if not allowed:
